...
export default {
  ...
  setup() {
    ...
    let goodInfos = computed(() => $store.state.detail.goodInfo);
    let categoryView = computed(() => getters.categoryView);
    let skuInfo = computed(() => getters.skuInfo);
    let spuSaleAttrList = computed(() => getters.spuSaleAttrList);
    let skuImageList = computed(() => getters.skuInfo.skuImageList || []);

    let changeActive = function (spuSaleAttrValue, arr) {
      _.forEach(arr, (item) => {
        item.isChecked = "0";
      });
