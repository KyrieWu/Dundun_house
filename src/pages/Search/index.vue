      ...
      <div class="bread">
        ...
        <ul class="fl sui-tag">
          ...
          <li class="with-x" v-if="searchParams.trademark">
            {{ searchParams.trademark }}<i>x</i>
          </li>
          <li
            class="with-x"
            v-for="(attrValue, idnex) in searchParams.props"
            :key="index"
          >
            {{ attrValue.split(":")[1] }}
...
export default {
  ...
  setup() {
    ...
    let getData = function () {
      $store.dispatch("reqGetSearchList", searchParams);
    };
    ...
    let removeAttr = function (index) {
      searchParams.props.splice(index, 1);
      getData();
    };

    let changeOrder = function (flag) {
    ...
    let getPageNo = (pageNo) => {
      searchParams.pageNo = pageNo;
      getData();
    };

    let goodsList = computed(() => getters.goodsList);
    let total = computed(() => $store.state.search.searchList);
